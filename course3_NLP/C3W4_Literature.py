...
# %%
max_length = max_sequence_len - 1  # -1 so to not include the label column
model = Sequential()
...
def predict_next_word(seed_txt):
    ...
    token_padded = pad_sequences([token_list], maxlen=max_length, padding='pre')
    print(token_padded.shape)
    predict_it = np.argmax(model.predict(token_padded, use_multiprocessing=True), axis=-1)
